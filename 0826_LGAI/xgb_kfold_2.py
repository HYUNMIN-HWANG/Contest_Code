...
for fold, (train_idx, valid_idx) in enumerate(skf.split(train_x, train_y)) :
    ...
    model = MultiOutputRegressor(xgb.XGBRegressor(n_estimators=100, learning_rate=0.07, gamma = 1, subsample=0.8, colsample_bytree = 0.8, max_depth=7, eval_set=eval_set) )
...
print(submit.head())
submit.to_csv(DATA_PATH + 'submit/kfold_2.csv', index=False)

...
simulation_sem_paths = sorted(glob.glob(DATA_PATH+'/simulation_data/SEM/*/*/*.png'))
simulation_depth_paths = sorted(glob.glob(DATA_PATH+'/simulation_data/Depth/*/*/*.png')+glob.glob(DATA_PATH+'/simulation_data/Depth/*/*/*.png'))
...
def inference(model, test_loader, device):
    ...
    os.chdir(DATA_PATH+"/submission/")
    sub_imgs = []
    for path, pred_img in zip(result_name_list, result_list):
        cv2.imwrite(path, pred_img)
        print(path)
        sub_imgs.append(path)
    submission = zipfile.ZipFile(DATA_PATH + "/submission.zip", 'w')
    for path in sub_imgs:
        submission.write(DATA_PATH+'/test/'+path)
    submission.close()
...
if __name__ == '__main__':
    ...
    test_sem_path_list = sorted(glob.glob(DATA_PATH + '/test/SEM/*.png'))
